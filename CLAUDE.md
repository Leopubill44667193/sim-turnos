...
| activo | boolean NOT NULL | default true. Se desactiva (no se borra la fila) desde la pestaña Fijos del admin |
| created_at | timestamptz NOT NULL | default now() |

**Índice:** `(negocio_id, dia_semana, activo)`

Turnos recurrentes por día de la semana (no confundir con los slots "fijos" con motivo de `slots_bloqueados`, que son bloqueos manuales de un solo día). Se gestionan desde la pestaña **Fijos** del admin vía `generarTurnosDesdeFijo(fijo, ocurrencias, desde)`, que busca las próximas `ocurrencias` fechas cuyo día de semana coincide con `dia_semana` a partir de `desde`, salteando las que ya tengan un turno (mismo `recurso_id` + `hora_inicio` + fecha), y por cada una crea (o reutiliza) el cliente por `(negocio_id, telefono)` e inserta en `turnos` con `email_verificacion: null` (por eso aparece en verde en la grilla, igual que un turno manual). Al **crear** un fijo se generan las próximas 4 ocurrencias desde hoy (defaults de la función). Al **renovar** un fijo se busca el último turno futuro existente para ese `recurso_id` + `hora_inicio` y se generan 4 ocurrencias más a partir de esa fecha (o desde hoy si no hay ninguno). Al **desactivar** un fijo se borran primero de `turnos` todos los turnos futuros (`fecha >= hoy`) que coincidan en `negocio_id` + `simulador_id` + `hora_inicio`, y recién después se marca `activo = false` en `turnos_fijos` (la fila del fijo no se borra). Migración ejecutada manualmente en Supabase (no vía script del repo).
